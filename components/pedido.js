...
const Pedido = ({ pedido }) => {
  const {
    id,
    total,
    cliente: { nombre, apellido, empresa, telefono, email },
    estado,
  } = pedido;
      ...
      cache.writeQuery({
        ...
        data: {
          obtenerPedidosVendedor: obtenerPedidosVendedor.filter(
            (perdidoActual) => perdidoActual.id !== id
          ),
        },
      });
    },
...
const confirmarEliminarPedido = () => {
  ...
  Swal.fire({
    title: `¿Seguro quiere eliminar este Pedido numero ${id} Monto Total RD! ${total} ?`,
  ...
  return (
    <div>
      ...
      >
        ...
        <div>
          <h2 className="text-gray-800 font-bold mt-2">Resumen del producto</h2>
          ...
          <button className="uppercase text-xs font-bold  flex item-center mt-4 bg-red-800 px-5 py-2 inline-block text-white rounded-tight" 
